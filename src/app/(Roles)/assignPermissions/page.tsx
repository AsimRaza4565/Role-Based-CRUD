  ...
  const ensureReadPermissions = (permissionIds: string[]) => {
    ...
    requiredReadSlugs.forEach((readSlug) => {
      const readPermission = permissions.find(
        (permission) => permission.slug === readSlug,
      );
      if (readPermission) {
    ...
    const fetchRolePermission = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const normalizedRolePermissionIds = ensureReadPermissions(
            rolePermissionIds || [],
          );
  ...
  const handleRoleUpdate = async () => {
    ...
    try {
      const normalizedPermissionIds =
        ensureReadPermissions(selectedPermissions);
                  ...
                  {roles?.map((role) => {
                    ...
                    return (
                      <li
                        ...
                        className={`cursor-pointer outline-none px-6 py-4 transition-all duration-200 ${
                          isSelected
                            ? "bg-indigo-50 shadow-[inset_4px_0_0_0_rgb(79_70_229)]"
                            : "hover:bg-slate-50"
                        }`}
                      >
                        <div className="flex flex-col">
                          <span
                            className={`font-semibold ${
                              isSelected ? "text-indigo-800" : "text-slate-900"
                            }`}
                          >
                            {role.name}
                          </span>
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                      <div className="flex items-center justify-between gap-2 lg:gap-4 mb-4 pb-4 border-b border-slate-100 flex-shrink-0">
                        ...
                        <button
                          ...
                          disabled={isUpdating}
                          className={`cursor-pointer md:w-[160px] flex justify-center items-center py-2.5 px-4 border border-transparent shadow-sm text-sm font-medium text-white transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${
                            !isUpdating
